...
from fast_kinda_solid_api.observability.tracing import (
    TraceContextModel,
    configure_tracing,
    span_context,
    span_function,
...
def setup_tracing():
    settings = OtelSettings(enable_console_exporter=True, enable_otel_exporter=False)
...
def test_configure_tracing_with_bad_otlp_protocol(setup_tracing):
    settings = OtelSettings(enable_console_exporter=False, enable_otel_exporter=True, otlp_protocol="asdf")
